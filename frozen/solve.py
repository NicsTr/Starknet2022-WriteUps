...
async def run():
    ...
    invocation = await contract.functions["readBalance"].call()
    print(invocation)

    for i in range(8):
        invocation = await contract.functions["deposit"].invoke(2**128-1, max_fee=int(1e16))
    ...
    invocation = await contract.functions["withdraw"].invoke(max_fee=int(1e16))
    await invocation.wait_for_acceptance()
    print(invocation)
